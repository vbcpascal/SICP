...
;36-scheme interpreter (parameter lazy/lazy-memo)
;vbcpascal
...
(define (let->combination exp)
  (cons (make-lambda
         (map car (let-clauses exp)) ; used to make the mistake of using list instead of con
        (let-body exp)) (map cadr (let-clauses exp))))

(define (definition-variable exp)
  (if (variable? (cadr exp))
      (cadr exp)    ;(define p 2)        =>  (cadr exp) is variable .
      (caadr exp))) ;(define (p x ) ..)  =>  (cadr exp) is a pair
...
;change  '((+ x 1) (+ x 2) (+ x 3)...)  into '(begin (+ x 1) (+ x 2) (+ x 3)...)
;change '((+ x 1)) into '(+ x 1)
